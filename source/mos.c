...
void mos_gd(HAL *hal,int addr){
  ...
  strncpy(&(hal->memory->MMEM[(int)addr/10][(int)addr%10]),hal->memory->LINE,40);
}
...
void mos_pd(HAL *hal,int addr){
  ...
  strncpy(hal->linep->OPLINE,&(hal->memory->MMEM[(int)addr/10][(int)addr%10]),40);
  hal->linep->OPFLAG=1;
}
...
void mos_bt(HAL *hal,int addr,int *row,int *line){
  ...
  if(hal->cpu->C==true){
    ...
    *row=(addr%10)*4-1;
    *line=(addr/10);
    printf("row=%d line=%d",*row+1,*line);
  }
}
